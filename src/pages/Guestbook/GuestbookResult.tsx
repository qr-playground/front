...
const GuestbookResult: React.FC = () => {
  ...
  const fetchGuestbooks = async (page: number) => {
    ...
    try {
      ...
      console.log("🔍 방명록 응답:", response);
      console.log("🔍 방명록 데이터:", response.data.guestbooks);
      if (response.data.guestbooks.length > 0) {
        console.log(
          "🔍 첫 번째 방명록 createdAt (원본):",
          response.data.guestbooks[0].createdAt
        );
      }
